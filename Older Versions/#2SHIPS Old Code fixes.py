...
def collide(rect1, rect2):
    hitboxes = set()
    hitboxes.add((rect1[0], rect1[1]))
    hitboxes.add((rect1[0] + rect1[2], rect1[1]))
    hitboxes.add((rect1[0] + rect1[2], rect1[1] + rect1[3]))
    hitboxes.add((rect1[0], rect1[1] + rect1[3]))

    for hitbox in hitboxes:
        if point_vs_rect(hitbox, rect2):
            return True
...
class Mouse:
    def __init__(self):
        self.buttons = [Key() for i in range(5)]

    def get_pressed(self):
        pressed = pygame.mouse.get_pressed(5)

        for i in range(5):
            set_new_states(self.buttons[i], pressed[i])
...
class Keyboard:
    def __init__(self):
        self.buttons = [Key() for i in range(512)]
...
def draw_roundsChoiceScreen(rounds, UI_newState):
    ...
    while True:
        ...
        m_pos = mouse.get_pos() + (1, 1)
        m_pressedButtons = mouse.get_pressed()
        
        if collide(m_pos, btn_roundsChoice_backToMainMenu.pos):
            if m_pressedButtons[0].pressed:
                change_UI_state(UI_STATE_MAINMENU)
                return ""
        ...
        if collide(m_pos, btn_roundsChoice_play.pos):
            if m_pressedButtons[0].pressed and len(rounds):
                change_UI_state(UI_newState)
                return int(rounds)
